...
fn build_window(app: &Application, cfg: &OsdConfig, palette: Palette, state: Arc<SharedState>) {
    ...
    // Layer-shell anchoring per position:
    //
    // - BottomCenter: anchor Bottom, margin_px from bottom edge, centered horizontally.
    // - TopCenter:    anchor Top,    margin_px from top edge,    centered horizontally.
    // - Corners:      anchor two edges (Top/Bottom + Left/Right), uniform margin_px on both.
    //
    // This replaces the swayosd-style fractional `top_margin` for centered positions,
    // which caused off-screen placement on monitors with wrongly-detected height.
    // The `top_margin` field is kept for config compatibility but is no longer used.
