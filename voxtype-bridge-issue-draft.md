...
How it works: after Whisper transcription (Groq `whisper-large-v3-turbo`, `language=auto`), when enabled it makes one Groq Chat call (`llama-3.3-70b`, same `remote_api_key`). One prompt does detection and translation: "if Russian translate to Spanish, if Spanish translate to Russian, output only translation." About 500ms. When off, it is straight transcription.

Built: `src/bridge/mod.rs` after `transcribe/remote.rs` — same `reqwest`, 15s timeout, fallback to original text when offline. Config `[bridge] enabled=false, pair="ru:es"` — disabled by default. Wired in `daemon.rs` after transcription. About 120 lines, `fmt/clippy/test` green. Branch `bridge` rebasing on `dev`. Short phrases like "sí" or "да" remain ambiguous, so fallback is passthrough.
